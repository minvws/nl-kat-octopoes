...
def schemavalidation(schema: str, expected_output: str) -> bool:
...
def test_schema__constrained_types_no_query__schemavalidationerror():
    assert schemavalidation("schema_custom_query.graphql", "Use of reserved type name is not allowed [type=Query]")
...
def test_schema__no_union_with_u__schemavalidationerror():
    assert schemavalidation("schema_no_union_with_u.graphql", "Self-defined unions must start with a U [type=Animals]")
